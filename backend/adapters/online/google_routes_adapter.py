...
class GoogleRoutesAdapter(DistanceMatrixAdapter):
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            raise DistanceMatrixRequestError(
                "Google Routes API key not provided.")
    ...
    def _to_latlon(coord: CoordinateType) -> Tuple[float, float]:
        ...
        if hasattr(coord, "lat") and hasattr(coord, "lon"):  # Support lon instead of lng
        ...
        if isinstance(coord, dict) and "lat" in coord and ("lng" in coord or "lon" in coord):
    ...
    async def get_matrix(self, request: MatrixRequest) -> MatrixResult:
        ...
        destinations_list = [self._to_latlon(d) for d in (
            request.destinations or request.origins)]
        ...
        request_body = {
            "origins": [{"location": {"latLng": {"latitude": lat, "longitude": lon}}} for lat, lon in origins_list],
            "destinations": [{"location": {"latLng": {"latitude": lat, "longitude": lon}}} for lat, lon in destinations_list],
