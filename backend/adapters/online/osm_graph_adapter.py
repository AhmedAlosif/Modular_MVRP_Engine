...
class OsmGraphAdapter(DistanceMatrixAdapter):
    ...
    def __init__(
        ...
        graph_factory: Optional[Callable[[float, float, int, str], nx.MultiDiGraph]] = None,
        node_locator: Optional[Callable[[nx.MultiDiGraph, List[Dict[str, float]]], List[int]]] = None,
    ):
    ...
    def build_matrix(self, coords: List[Dict[str, float]], mode: str = "driving") -> MatrixResult:
        return self.build(coords, coords, mode)

    # Common aliases other parts of the code may look for
    def matrix(self, coords: List[Dict[str, float]], mode: str = "driving") -> MatrixResult:
        return self.build_matrix(coords, mode)

    def distance_matrix(self, coords: List[Dict[str, float]], mode: str = "driving") -> MatrixResult:
        return self.build_matrix(coords, mode)

    def fetch_matrix(self, coords: List[Dict[str, float]], mode: str = "driving") -> MatrixResult:
        return self.build_matrix(coords, mode)

    # ── Two-list rectangular matrix (origins × destinations) ───────────────────
    def get_matrix(self, origins: List[Dict[str, float]], destinations: Optional[List[Dict[str, float]]] = None, mode: str = "driving") -> MatrixResult:
