...
def check_time_windows_length(tw: Optional[List[Optional[List[int]]]], n: int) -> Optional[str]:
    if tw is None:
        return None
    if len(tw) != n:
...
def check_capacity_feasibility(demands: Optional[List[int]], vehicle_caps: List[int]) -> Optional[str]:
    if demands is None:
        return None
...
def validate_instance(instance: Dict[str, Any]) -> List[str]:
    ...
    caps = [int((v.get("capacity") or [10**9])[0]) for v in vehicles] if vehicles else [10**9]
    cap_msg = check_capacity_feasibility(instance.get("demands"), caps)
    if cap_msg: issues.append(cap_msg)
