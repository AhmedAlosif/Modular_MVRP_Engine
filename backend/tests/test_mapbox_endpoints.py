...
def test_mapbox_matrix_ok():
    try:
        respx.get("https://api.mapbox.com/directions-matrix/v1/mapbox/driving/*").mock(
            return_value=httpx.Response(200, json={"distances": [[0,1234],[1234,0]], "durations": [[0,60],[60,0]]})
        )
        r = client.post("/mapbox/matrix", json={
            "profile":"driving",
...
def test_mapbox_optimize_ok():
    try:
        respx.get("https://api.mapbox.com/optimized-trips/v1/mapbox/driving/*").mock(
            return_value=httpx.Response(200, json={"code":"Ok","trips":[{"distance":1000,"duration":120}],"waypoints":[]})
        )
        r = client.post("/mapbox/optimize", json={
            "profile":"driving",
            "coordinates":[{"lon":10,"lat":20},{"lon":11,"lat":21}],
...
def test_mapbox_match_ok():
    try:
        respx.get("https://api.mapbox.com/mapbox/map-matching/v5/mapbox/driving/*").mock(
            return_value=httpx.Response(200, json={"code":"Ok","matchings":[{"geometry":{"type":"LineString","coordinates":[[10,20],[11,21]]}}]})
        )
        r = client.post("/mapbox/match", json={
            "profile":"driving",
            "coordinates":[{"lon":10,"lat":20},{"lon":11,"lat":21}]
        })
        assert r.status_code == 200
...
def test_mapbox_geocode_ok():
    try:
        respx.get("https://api.mapbox.com/geocoding/v5/mapbox.places/*.json").mock(
            return_value=httpx.Response(200, json={"features":[{"place_name":"Test Place"}]})
        )
