...
def test_vrp_set_xml100_pair(client):
    ...
    r = client.get("/benchmarks/find", params={
        "dataset": XML_DATASET,
        "name": XML_NAME
    })
    ...
    # Same base name?
    assert os.path.splitext(instance["name"])[0] == os.path.splitext(solution["name"])[0]
...
def test_solomon_c101_listed(client):
    ...
    assert any(n.lower().endswith(SOLOMON_TARGET) for n in names), f"{SOLOMON_TARGET} not listed"
